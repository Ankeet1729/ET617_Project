...
def clear_tables(cursor):
    ...
    print("🧹 Clearing existing data from submodule and concept tables...")
    cursor.execute("TRUNCATE TABLE submodules, concepts, concept_grade_mapping RESTART IDENTITY CASCADE;")
...
def seed_data():
    ...
    with conn.cursor() as cursor:
        ...
        if not json_files:
            print("❌ No JSON files found in 'output/'. Please run generate_concept_subsets.py first.")
            return
        ...
        for file_path in json_files:
            ...
            # Insert submodule
            transcript_path = TRANSCRIPT_DIR / f"{submodule_code}_combined_transcript.txt"
            image_path = IMAGE_DIR / f"{submodule_code}_image.png"
            try:
            ...
            cursor.execute(
                "INSERT INTO submodules (submodule_code, submodule_name, image_path, transcript) VALUES (%s, %s, %s, %s) RETURNING id;",
            ...
            for concept in data.get("concepts", []):
                ...
                # Handle missing ct_concepts by falling back to description
                ct_concepts = concept.get("ct_concepts", [concept.get("concept_description", "")])
                description = concept.get("concept_description") or concept.get("description")
                
                cursor.execute(
                    "INSERT INTO concepts (submodule_id, concept_name, ct_concepts, description) VALUES (%s, %s, %s, %s) RETURNING id;",
                ...
                if not grade_range:
                    print(f"   ⚠️ WARNING: Concept '{concept_name}' has no valid grade range. It will NOT be mapped.")
                    continue

                for grade in grade_range:
                    cursor.execute(
                        "INSERT INTO concept_grade_mapping (concept_id, grade) VALUES (%s, %s) ON CONFLICT DO NOTHING;",
                        (concept_id, grade)
                    )
            print(f"   - Processed {len(data.get('concepts', []))} concepts for submodule {submodule_code}")
